...
protected static final int MAX_APPARENT_POWER = 40000; // Beispiel, anpassen

protected static final int NET_CAPACITY = 10000; // Beispiel, hardcoded

private static final int MIN_REACTIVE_POWER = -10000; // Beispiel

private static final int MAX_REACTIVE_POWER = 10000; // Beispiel

private static final int POWER_LIMIT_HYSTERESIS = 500; // 500 W Hysterese
...
* Nominal power of the inverter in Watts. Used for converting set-points to percent.

*/

private static final double NOMINAL_POWER_WATTS = 15000.0; // 15 kW



/**

* Scaling factor for percentage values written to Modbus.

* Assumes the register expects values in 0.1 % units (e.g., 105 means 10.5%).
...
// === Listener für Leistungslimits (Angepasst für neg. Charge Limit) ===
...
// applyPower() - Mit Prozent-Umrechnung
...
// --- Umrechnung von Watt/VAR in Prozent (skaliert auf 0.1%) ---

int activePowerPercentScaled = (int) Math.round(activePower / (NOMINAL_POWER_WATTS / (100.0 * PERCENT_SCALING_FACTOR)));

int reactivePowerPercentScaled = (int) Math.round(reactivePower / (NOMINAL_POWER_WATTS / (100.0 * PERCENT_SCALING_FACTOR)));



// Optional: Logge die Umrechnung für Debugging

// log.info("applyPower received [W/var]: P={}, Q={}. Calculated Scaled Percent [0.1%]: P={}, Q={}",

// activePower, reactivePower, activePowerPercentScaled, reactivePowerPercentScaled);



// Setze die UM GERECHNETEN skalierten Prozentwerte
...
// Tasks bleiben gleich, da sie Integer schreiben
...
new FC3ReadRegistersTask(636, Priority.HIGH, m(SymmetricEss.ChannelId.ACTIVE_POWER, new SignedWordElement(636))), // Gemessene Leistung in W
...
new FC16WriteRegistersTask(1111, m(DeyeSunHybrid.ChannelId.SET_ACTIVE_POWER, new SignedWordElement(1111))), // Schreibt skalierten %-Wert

new FC16WriteRegistersTask(1118, m(DeyeSunHybrid.ChannelId.SET_REACTIVE_POWER, new SignedWordElement(1118))) // Schreibt skalierten %-Wert
...
String origCharge = this.channel(DeyeSunHybrid.ChannelId.ORIGINAL_ALLOWED_CHARGE_POWER).value().asOptional().map(String::valueOf).orElse("N/A"); // Negativ [W]

String origDischarge = this.channel(DeyeSunHybrid.ChannelId.ORIGINAL_ALLOWED_DISCHARGE_POWER).value().asOptional().map(String::valueOf).orElse("N/A"); // Positiv [W]

String finalCharge = this.getAllowedChargePower().asOptional().map(String::valueOf).orElse("N/A"); // Negativ [W]

String finalDischarge = this.getAllowedDischargePower().asOptional().map(String::valueOf).orElse("N/A"); // Positiv [W]
...
// Zeige den NÄCHSTEN zu schreibenden skalierten Prozentwert an
...
nextWriteActivePercent = setActivePowerChannel.getNextWriteValue().map(String::valueOf).orElse("N/A"); // .asOptional() entfernt



IntegerWriteChannel setReactivePowerChannel = this.channel(DeyeSunHybrid.ChannelId.SET_REACTIVE_POWER);

nextWriteReactivePercent = setReactivePowerChannel.getNextWriteValue().map(String::valueOf).orElse("N/A"); // .asOptional() entfernt

} catch (ClassCastException e) {

log.warn("Unable to cast SET_ACTIVE/REACTIVE_POWER to IntegerWriteChannel in debugLog");
...
+ "|NextWrite(0.1%):[P:" + nextWriteActivePercent + ";Q:" + nextWriteReactivePercent + "]"; // Angepasst
...
// Normale Logik, kein Testcode mehr
...
// isManaged() - Wieder Original-Logik
...
// Angepasste Präzision basierend auf 0.1% von 15kW

@Override

public int getPowerPrecision() {

// Kleinster Schritt ist 0.1% von 15000W = 15W
